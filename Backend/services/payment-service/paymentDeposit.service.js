...
import {getOrcreateSystemWalletAccount, getOrCreateUserWalletAccounts,} from "../wallet-ledger-service/walletAccount.service.js";
import { postLedgerTransaction } from "../wallet-ledger-service/ledger.service.js";
import {createRazorpayOrder, verifyRazorpayCheckoutSignature,} from "./razorpay.service.js";

const DEFAULT_FIAT_CURRENCY = process.env.RAZORPAY_CURRENCY || "INR";
const DEFAULT_USDT_RATE = process.env.RAZORPAY_USDT_RATE || "1";
const RAZORPAY_ORDER_EXPIRY_MINUTES = Number(process.env.RAZORPAY_ORDER_EXPIRY_MINUTES || 15);
...
const createFiatDepositOrder = async ({ userId, amount }) => {
    ...
    const order = await createRazorpayOrder({
      ...
      receipt: `fiat-deposit:${userId}:${Date.now()}`,
      notes: {
        userId,
